...
def get_pnp_trends(time_filter, municipality=None):
    try:
        if time_filter == 'today':
            ...
            for response in today_responses:
                ...
                if response_time.date() == today and response.get('municipality', '').lower() == municipality.lower() and response.get('role', '').lower() == 'pnp':
...
def get_pnp_distribution(time_filter, municipality=None):
    try:
        if time_filter == 'today':
            from AlertNow import today_responses
            distribution = Counter(response.get('emergency_type', 'Unknown') for response in today_responses if response.get('municipality', '').lower() == municipality.lower() and response.get('role', '').lower() == 'pnp')
            return {k: {'total': v, 'responded': sum(1 for r in today_responses if r.get('emergency_type', '') == k and r.get('municipality', '').lower() == municipality.lower() and r.get('role', '').lower() == 'pnp' and r.get('responded', False))} for k, v in distribution.items()}
        else:
            mock_data = generate_mock_data(time_filter, 'road') + generate_mock_data(time_filter, 'fire')
            distribution = Counter(record.get('emergency_type', 'Unknown') for record in mock_data if record.get('municipality', '').lower() == municipality.lower() and record.get('role', '').lower() == 'pnp')
            return {k: {'total': v, 'responded': sum(1 for r in mock_data if r.get('emergency_type', '') == k and r.get('municipality', '').lower() == municipality.lower() and r.get('role', '').lower() == 'pnp' and r.get('responded', False))} for k, v in distribution.items()}
    except Exception as e:
        logger.error(f"Error in get_pnp_distribution: {e}")
        return {'Unknown': {'total': 0, 'responded': 0}}

def get_pnp_causes(time_filter, municipality=None):
    try:
        if time_filter == 'today':
            today = datetime.now(pytz.timezone('Asia/Manila')).date()
            road_causes = Counter()
