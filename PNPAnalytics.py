...
def get_pnp_trends(time_filter, municipality=''):
    try:
        ...
        if municipality and 'municipality' in road_df.columns:
            road_df = road_df[road_df['municipality'] == municipality]
...
def get_pnp_accident_types(time_filter, municipality=''):
    ...
    try:
        response_data = [r for r in responses if r.get('role') == 'pnp' and (not municipality or r.get('municipality', '').lower() == municipality.lower())]
        ...
        if time_filter != 'yearly':
            ...
            df['timestamp'] = pd.to_datetime(df['timestamp'])
            df = df[(df['timestamp'] >= start_date) & (df['timestamp'] <= end_date)]
        accident_types = Counter(df['accident_type'])
