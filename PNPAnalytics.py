    ...
    try:
        if time_filter == 'today':
            ...
            for response in today_responses:
                ...
                if response_time.date() == today and (municipality is None or response.get('municipality', '').lower() == municipality.lower()) and response.get('role', '').lower() == 'pnp':
            ...
            for record in mock_data:
                if isinstance(record, str):
                    try:
                        ...
                        continue
                if (municipality is None or record.get('municipality', '').lower() == municipality.lower()) and record.get('role', '').lower() == 'pnp':
            ...
            for response in today_responses:
                if isinstance(response, str):
                    try:
                        ...
                        logger.warning(f"Skipping invalid JSON response: {response}")
                        continue
                if (municipality is None or response.get('municipality', '').lower() == municipality.lower()) and response.get('role', '').lower() == 'pnp':
            ...
            return {k: {'total': v, 'responded': sum(1 for r in today_responses
                                                    if (isinstance(r, dict) and r.get('emergency_type', '') == k and
                                                       ...
                                                       (isinstance(r, str) and json.loads(r).get('emergency_type', '') == k and
                                                        ...
                                                        json.loads(r).get('role', '').lower() == 'pnp' and
...
def get_accident_type(time_filter, municipality=None):
    try:
        if time_filter == 'today':
            today = datetime.now(pytz.timezone('Asia/Manila')).date()
            ...
            for response in today_responses:
                ...
                if response_time.date() == today and response.get('emergency_type', '') == 'Road Accident' and \
                   (municipality is None or response.get('municipality', '').lower() == municipality.lower()) and \
                   response.get('role', '').lower() == 'pnp':
            ...
            for record in mock_data:
                ...
                if record.get('emergency_type', '') == 'Road Accident' and \
                   (municipality is None or record.get('municipality', '').lower() == municipality.lower()) and \
                   record.get('role', '').lower() == 'pnp':
...
def get_road_condition(time_filter, municipality=None):
    try:
        if time_filter == 'today':
            today = datetime.now(pytz.timezone('Asia/Manila')).date()
