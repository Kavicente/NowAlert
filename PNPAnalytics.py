...
def get_pnp_analytics_data(time_filter, municipality):
    try:
        ...
        query = '''
            ...
            FROM pnp_response
            WHERE barangay IN (SELECT barangay FROM barangay_coords WHERE municipality = ?) 
            AND emergency_type = 'road' AND timestamp BETWEEN ? AND ? AND responded = 1
        '''
        params = (municipality.lower(), start_time, end_time)
        
        if time_filter == 'daily':
            query = query.replace('timestamp BETWEEN ? AND ?', 'DATE(timestamp) = DATE(?)')
            params = (municipality.lower(), start_time)
        ...
        print(f"Error in get_pnp_analytics_data: {e}")
        return {
            'trends': {'labels': [], 'total': [], 'responded': []},
            'distribution': {},
            'causes': {'road': {}, 'fire': {}},
