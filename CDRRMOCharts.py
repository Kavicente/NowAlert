...
def get_cdrrmo_chart_data(time_filter, barangay=None):
    ...
    c = conn.cursor()
    query = "SELECT road_accident_cause, road_accident_type, driver_gender, vehicle_type, driver_age, road_condition, barangay, timestamp FROM cdrrmo_response"
...
def cdrrmo_charts():
    from AlertNow import app  # Moved inside function
    return render_template('CDRRMOCharts.html', current_datetime=datetime.now(pytz.timezone('Asia/Manila')).strftime('%Y-%m-%d %H:%M:%S'))
